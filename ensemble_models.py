...
def main():
    ...
    pretrained_list = [
        "EfficientNetB0"
    ]  # , 'MobileNetV2']#['MobileNetV2', 'VGG16', 'ResNet50']
    ...
    for model_name in pretrained_list:
        ...
        print(
            f"Our {model_name} has achieved an accuracy of  {acc} in {len(hist.epoch)} epoch(s)"
        )
        history_finetune = fine_tune_model(
            base_model, model, ds_train, ds_valid, fine_tune_at=2, lr=1e-6
        )
